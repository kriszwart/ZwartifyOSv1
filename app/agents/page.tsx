      ...
      <div className="relative z-10 min-h-screen">
            ...
            <div className="bg-black/50 border-2 border-green-400/50 p-6 rounded-lg mb-6">
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-400/60 mt-1">
                    Select Skills to give this agent specialized capabilities. Skills provide domain expertise and workflows.
                  </p>
                </div>
                <button
                  type="submit"
                        ...
                        {agent.createdByAgentId && (
                          <button
                            onClick={() => {
                              ...
                              if (agent.creationPrompt) {
                                setSelectedCreationPrompt(agent.creationPrompt)
                              } else {
                                setSelectedCreationPrompt(`Created by ${creatorAgent?.name || 'an AI agent'}`)
                              }
                            }}
                            className="px-2 py-1 text-xs font-mono border border-purple-400/50 bg-purple-400/10 text-purple-400 hover:bg-purple-400/20 transition-colors cursor-pointer"
        ...
        {selectedCreationPrompt && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-black border-2 border-green-400/50 rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-bold text-green-400 font-mono">Original Creation Prompt</h3>
                <button
                  onClick={() => setSelectedCreationPrompt(null)}
                  className="text-green-400 hover:text-green-300 transition-colors font-mono text-2xl"
                >
                  ×
                </button>
              </div>
              <div className="bg-black/50 border border-green-400/30 p-4 rounded font-mono text-sm text-green-300 whitespace-pre-wrap">
                {selectedCreationPrompt}
              </div>
              <div className="mt-4 text-xs text-green-400/60 font-mono">
                This shows the original prompt used to create this agent via G-SAC.
              </div>
            </div>
          </div>
        )}
